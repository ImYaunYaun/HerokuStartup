...
$page_data = mysqli_query( $cralwer,$query_limit_page_data);
$row_page_data = mysqli_fetch_assoc($page_data);

if (isset($_GET['totalRows_page_data'])) {
  $totalRows_page_data = $_GET['totalRows_page_data'];
} else {
  $all_page_data = mysqli_query($query_page_data);
  $totalRows_page_data = mysqli_num_rows($all_page_data);
}
...
<body>
  ...
  <?php do {
    ...
    $totalRows_webinfo = mysqli_num_rows($webinfo);
  ?>
    <div class="resultDiv">
  ...
  <?php } while ($row_favorite = mysqli_fetch_assoc($favorite)); ?>
